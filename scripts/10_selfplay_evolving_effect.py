...
def main():
    ...
    if args.ablation == "freeze_g":
        frozen_adv = generate_adv_batch(
            train_rows, level=args.adv_level, seed=args.seed
        )
    ...
    with log_path.open("w", newline="") as f:
        ...
        for r in range(1, args.rounds + 1):
            ...
            if frozen_adv is not None:
                ...
                adv_rows = generate_adv_batch(
                    train_rows, level=args.adv_level, seed=args.seed + 1000 * r
                )
            ...
            hardness = sum((la - lc) for la, lc in zip(loss_adv, loss_clean)) / max(
                1, len(loss_adv)
            )
